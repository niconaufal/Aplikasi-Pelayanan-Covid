...
<thead>
    ...
    <?php
        ...
        foreach($dataProv as $dp){
            ...
            $jumlahPria = mysqli_query($conn,"SELECT COUNT(nama) as jumlahPria FROM tbl_register WHERE provinsi='$dp[nama]' AND jk='pria' ");
                $totalPria = mysqli_fetch_array($jumlahPria);
            $jumlahWanita = mysqli_query($conn,"SELECT COUNT(nama) as jumlahWanita FROM tbl_register WHERE provinsi='$dp[nama]' AND jk='wanita' ");
                $totalWanita = mysqli_fetch_array($jumlahWanita);

            echo"<tr>
                <td>$no</td>
                <td>$dp[nama]</td>
                <td align='center'>$totalPria[jumlahPria]</td>
                <td align='center'>$totalWanita[jumlahWanita]</td>
            </tr>";
            $no++;
        } 
    ?>
